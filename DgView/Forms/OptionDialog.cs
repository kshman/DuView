...
public class OptionDialog : Dialog
{
    ...
    public OptionDialog(Window? parent) : base("설정", parent, DialogFlags.Modal)
    {
        ...
        _useMagneticCheck = new CheckButton("자석 윈도우 사용");
        _confirmDeleteCheck = new CheckButton("파일을 지울때 확인");
        _alwaysTopCheck = new CheckButton("프로그램을 맨 위에 보이기");
        ...
        generalBox.PackStart(new Label("캐시 크기"), false, false, 0);
        generalBox.PackStart(cacheBox, false, false, 0);
        generalBox.PackStart(new Label("외부 실행"), false, false, 0);
        ...
        optionTab.AppendPage(kmBox, new Label("키보드/마우스"));
