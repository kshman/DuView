...
public class RenexDialog : Dialog
{
	...
	public RenexDialog(Gtk.Window? parent, string filename) :
		base("책 이름 바꾸기", parent, DialogFlags.Modal)
	{
		#region 디자인
		// 대화상자
		...
		Icon = ResL.GetBitmap(ResL.IconRename);
	...
	{
		if (args.Event.Key is GdkKey.Return or GdkKey.KP_Enter)
		{
			// 엔터 키가 눌렸을 때
		...
		else if (args.Event.Key == GdkKey.Escape)
		{
			// ESC 키가 눌렸을 때
